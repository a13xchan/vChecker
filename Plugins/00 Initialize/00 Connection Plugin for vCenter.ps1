...
#Get the password
$PasswordFile = "test_readcred.txt"
$securePassword = Get-Content D:\vChecker-Test\$PasswordFile | ConvertTo-SecureString
...
if ($OpenConnection.IsConnected) {
    ...
    $VIConnection = Connect-VIServer -Server $VIServer -Credential $credentials
}
...
#Get the CIS user account password
$CISPasswordFile = "test_adminlocalcred.txt"
$secureCISPassword = Get-Content D:\vChecker-Test\$CISPasswordFile | ConvertTo-SecureString
$CIScredentials = New-Object System.Management.Automation.PSCredential ($CISUser, $secureCISPassword)


# Adding Connection to CIS Server
$OpenConnection = $global:DefaultCISServers | Where-Object { $_.Name -eq $VIServer }
if ($OpenConnection.IsConnected) {
    Write-CustomOut ( "{0}: {1}" -f $pLang.connReuse, $Server )
    $CISConnection = $OpenConnection
}
else {
    Write-CustomOut ( "{0}: {1}" -f $pLang.connOpen, $Server )
    $CISConnection = Connect-CisServer -Server $VIServer -Credential $CIScredentials
}
if (-not $CISConnection.IsConnected) {
    Write-Error $pLang.connError
}
...
Function Get-VSANVersion {
    <#
        ...
        .PARAMETER paramCluster
            Cluster View Object
        .EXAMPLE
            Get-VSANVersion -paramCluster VSAN-Cluster
    #>
    param(
        [Parameter(Mandatory = $true)]$paramCluster
    )
    $vchs = Get-VSANView -Id "VsanVcClusterHealthSystem-vsan-cluster-health-system"
    $cluster_view = $paramCluster.MoRef
    $results = $vchs.VsanVcClusterQueryVerifyHealthSystemVersions($cluster_view)
    
    #Write-Host "`nVC Version:"$results.VcVersion
    $results.HostResults | Select-Object Hostname, Version
}
